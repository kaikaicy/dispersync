...
const purposeOptions = [
  { label: 'Transfer of Ownership', key: 'ownership', icon: 'swap-horizontal' },
  { label: 'Relocation', key: 'relocation', icon: 'navigate' },
...
export default function Transfer({ onBackToTransactions, navigation }) {
  ...
  const handleSubmit = () => {
    ...
    if (!animalDetails.quantity.trim()) { Alert.alert('Validation Error', 'Please enter the quantity.'); return; }
    if (!animalDetails.speciesItem.trim()) { Alert.alert('Validation Error', 'Please enter the species/item.'); return; }
    if (!selectedPurpose) { Alert.alert('Validation Error', 'Please select a purpose of transfer.'); return; }
    if (selectedPurpose === 'others' && !otherPurpose.trim()) { Alert.alert('Validation Error', 'Please specify the other purpose.'); return; }
    ...
    setTimeout(() => {
      ...
      setAnimalDetails({ quantity: '', speciesItem: '', breedType: '', ageWeight: '', sex: '', remarks: '' });
      setSelectedPurpose('');
      setOtherPurpose('');
      setSearchTerm('');
      ...
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent}>
        <View style={styles.container}>
          ...
          <View style={styles.section}>
            ...
            <TextInput style={styles.input} placeholder="Enter quantity" placeholderTextColor={colors.textLight} value={animalDetails.quantity} onChangeText={(text) => setAnimalDetails(prev => ({ ...prev, quantity: text }))} keyboardType="numeric" />

            <Text style={styles.inputLabel}>Species/Item:</Text>
            <TextInput style={styles.input} placeholder="Enter species or item" placeholderTextColor={colors.textLight} value={animalDetails.speciesItem} onChangeText={(text) => setAnimalDetails(prev => ({ ...prev, speciesItem: text }))} />

            <Text style={styles.inputLabel}>Breed/Type:</Text>
            <TextInput style={styles.input} placeholder="Enter breed or type" placeholderTextColor={colors.textLight} value={animalDetails.breedType} onChangeText={(text) => setAnimalDetails(prev => ({ ...prev, breedType: text }))} />

            <Text style={styles.inputLabel}>Age/Weight:</Text>
            <TextInput style={styles.input} placeholder="Enter age or weight" placeholderTextColor={colors.textLight} value={animalDetails.ageWeight} onChangeText={(text) => setAnimalDetails(prev => ({ ...prev, ageWeight: text }))} />

            <Text style={styles.inputLabel}>Sex:</Text>
            <TextInput style={styles.input} placeholder="Enter sex (M/F)" placeholderTextColor={colors.textLight} value={animalDetails.sex} onChangeText={(text) => setAnimalDetails(prev => ({ ...prev, sex: text }))} />

            <Text style={styles.inputLabel}>Remarks:</Text>
            <TextInput style={styles.remarksInput} placeholder="Enter remarks" placeholderTextColor={colors.textLight} value={animalDetails.remarks} onChangeText={(text) => setAnimalDetails(prev => ({ ...prev, remarks: text }))} multiline numberOfLines={3} />
          </View>

          {/* Purpose of Transfer Section */}
          <View style={styles.section}>
            <View style={styles.sectionHeader}>
              <Text style={styles.sectionTitle}>Purpose of Transfer</Text>
            </View>
            <View style={styles.purposeOptionsGrid}>
              {purposeOptions.map((opt) => {
                const isSelected = selectedPurpose === opt.key;
                return (
                  <TouchableOpacity key={opt.key} style={[styles.purposeOptionCard, isSelected && styles.purposeOptionCardSelected]} onPress={() => handlePurposeSelection(opt.key)} activeOpacity={0.7}>
                    <View style={styles.purposeOptionContent}>
                      <View style={[styles.purposeIconContainer, isSelected && styles.purposeIconContainerSelected]}>
                        <Ionicons name={opt.icon} size={18} color={isSelected ? colors.white : colors.textLight} />
                      </View>
                      <Text style={[styles.purposeOptionLabel, isSelected && styles.purposeOptionLabelSelected]}>{opt.label}</Text>
                      <View style={[styles.purposeRadio, isSelected && styles.purposeRadioSelected]}>
                        {isSelected && (<View style={styles.purposeRadioInner} />)}
                      </View>
                    </View>
                  </TouchableOpacity>
                );
            ...
            {selectedPurpose === 'others' && (
              <View style={{ marginTop: 16 }}>
                ...
                <TextInput style={styles.input} placeholder="Specify other purpose" placeholderTextColor={colors.textLight} value={otherPurpose} onChangeText={setOtherPurpose} />
              </View>
            )}
          </View>
...
const styles = StyleSheet.create({
  ...
  remarksInput: { backgroundColor: '#FFFFFF', borderRadius: 10, paddingHorizontal: 14, paddingVertical: 12, fontSize: 15, borderWidth: 1, borderColor: '#E3F4EC', color: '#25A18E', minHeight: 80, textAlignVertical: 'top' },
  purposeOptionsGrid: { gap: 12, marginBottom: 16 },
  purposeOptionCard: { backgroundColor: '#FFFFFF', borderRadius: 12, padding: 16, borderWidth: 1, borderColor: '#E3F4EC' },
  purposeOptionCardSelected: { borderColor: '#25A18E', backgroundColor: '#e6f4f1' },
  purposeOptionContent: { flexDirection: 'row', alignItems: 'center' },
  purposeIconContainer: { width: 36, height: 36, borderRadius: 18, backgroundColor: '#f8f9fa', alignItems: 'center', justifyContent: 'center', marginRight: 12 },
  purposeIconContainerSelected: { backgroundColor: '#25A18E' },
  purposeOptionLabel: { fontSize: 15, color: '#25A18E', fontWeight: '500', flex: 1 },
  purposeOptionLabelSelected: { color: '#25A18E', fontWeight: '600' },
  purposeRadio: { width: 20, height: 20, borderRadius: 10, borderWidth: 2, borderColor: '#25A18E', backgroundColor: '#FFFFFF', alignItems: 'center', justifyContent: 'center' },
  purposeRadioSelected: { backgroundColor: '#25A18E', borderColor: '#25A18E' },
  purposeRadioInner: { width: 8, height: 8, borderRadius: 4, backgroundColor: '#FFFFFF' },
